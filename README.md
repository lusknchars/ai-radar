...
`equations`. The step runs for every paper without a status row, so a fetch
that failed one day is retried the next. `scripts/backfill_equations.py`
runs the same step by hand.

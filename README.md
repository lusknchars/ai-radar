...
The interface reuses the authored Halo family from `frontend-lab`: the
SheenButton interaction for report actions, MetricTile-style collection totals,
and InsightNote-style calculated observations. They are ported to semantic HTML
and plain CSS because this site is generated by Python. There is still no React
bundle, client build, remote font, or component-library dependency.

The paper queue now comes before the charts. Desktop keeps the comparison table;
mobile turns the same semantic rows into readable paper cards. Internal values
such as `cache_kv` and `single_gpu_24gb` remain stable in data and filters, while
the interface renders labels such as “cache KV” and “1 GPU, até 24 GB”.
